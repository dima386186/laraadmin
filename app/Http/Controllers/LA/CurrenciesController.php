...
class CurrenciesController extends Controller
{
	...
	{
		if(Module::hasAccess("Currencies", "create")) {
	...
	{
		if(Module::hasAccess("Currencies", "view")) {
	...
	{
		if(Module::hasAccess("Currencies", "edit")) {			
			$currency = Currency::find($id);
			if(isset($currency->id)) {	
				$module = Module::get('Currencies');
	...
	{
		$values = DB::table('currencies')->select($this->listing_cols)->whereNull('deleted_at');
